...
| 2026-06-18 | Auto-export delivered work to student GitHub/Drive/portfolio/integrations | Accepted with a consent guardrail; scaffolding pending | Founder wants student work auto-archived to all their connected destinations on completion. ENGINEERING/LEGAL CONSTRAINT: raw deliverables are client-owned (work-for-hire) and may contain confidential data/PII, so they default to PRIVATE and only reach PUBLIC destinations when the client marks the order shareable / grants a portfolio license; public destinations otherwise receive the IP-safe case-study, not raw files. Trigger fires only after AI-gate pass + client approval + payout settle. A true no-consent public dump is explicitly OUT until legal review. |
