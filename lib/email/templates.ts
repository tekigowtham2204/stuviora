...
export function orderHiredEmail(p: {
  ...
  const bodyHtml = `Hi ${escapeHtml(p.studentName)},<br><br>A client just hired you for "${escapeHtml(p.jobTitle)}". Their payment of ${inr(p.amount)} is authorized and guaranteed, so you can start with confidence. You keep 85%, charged to the client and settled to you once your work passes the AI gate.`;
  const text = `Hi ${p.studentName},\n\nA client just hired you for "${p.jobTitle}". Their payment of ${inr(p.amount)} is authorized and guaranteed, so you can start with confidence. You keep 85%, charged to the client and settled to you once your work passes the AI gate.\n\nOpen the order: ${APP}/student/orders/${encodeURIComponent(p.orderId)}`;
  return {
    subject,
    text,
    html: layout({
...
export function orderSubmittedEmail(p: {
  ...
  const subject = `Delivery ready: ${oneLine(p.jobTitle)}`;
  const bodyHtml = `Hi ${escapeHtml(p.clientName)},<br><br>Your freelancer's delivery for "${escapeHtml(p.jobTitle)}" passed our AI quality check, so it has been delivered and your authorized payment was captured. If anything is off, request a fix within 72 hours and you are refunded while it is reworked. Otherwise it settles to the student automatically.`;
  const text = `Hi ${p.clientName},\n\nYour freelancer's delivery for "${p.jobTitle}" passed our AI quality check, so it has been delivered and your authorized payment was captured. If anything is off, request a fix within 72 hours and you are refunded while it is reworked. Otherwise it settles to the student automatically.\n\nReview the delivery: ${APP}/client/orders/${encodeURIComponent(p.orderId)}`;
  return {
    subject,
    text,
    html: layout({
